...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // const [FilteredProds, setFilteredProd] = useState<Product[]>(TempProd);

  function Clear() {
    setSelectedIdBrand(null);
    setSelectedIdCateg(null);
    setSelectedId(null);
    setProd(TempProd);
  }
          ...
          <div style={{ height: "85%" }} className="flex  ">
            <div className="bg-white  text-black w-1/2 ">
              {options.map((option) => (
                <div
                  key={option.id}
                  className={`flex flex-row justify-between px-4 py-8 cursor-pointer hover:bg-gray-100  font-semibold text-lg ${
                    selected === option.id
                      ? "bg-gray-100 "
                      : "hover:bg-gray-700"
                  }`}
                  ...
                  <span>
                    {option.id === 1
                      ? selectedId?.length === 0
                        ...
                        : selectedId?.length
                      : option.id === 2
                      ? selectedIdBrand?.length === 0
                        ? ""
                        : selectedIdBrand?.length
                      : option.id === 3
                      ? selectedIdCateg?.length === 0
                        ? ""
                        : selectedIdCateg?.length
                      : "error"}
                  </span>
                </div>
              ))}
                ...
                allSizes.map((item, Index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <Check
                        color={`${
                          selectedId?.includes(Index) ? "red" : "gray"
                        }`}
                      />
                      {item}
                    </div>
              ...
              {selected === 2 &&
                allBrands.map((item, index) => (
                  <div
                    ...
                    className={`flex justify-between items-center px-4 py-8 cursor-pointer
                      ${
                        selectedIdBrand?.includes(index)
                          ? "bg-white text-black font-semibold"
                          : "bg-gray-100"
                      }`}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <Check
                        color={`${
                          selectedIdBrand?.includes(index) ? "red" : "gray"
                        }`}
                      />
                      {item}
                    </div>
              ...
              {selected === 3 &&
                allSubCateg.map((item, index) => (
                  <div
                    ...
                    className={`flex justify-between items-center px-4 py-8 cursor-pointer
                      ${
                        selectedIdCateg?.includes(index)
                          ? "bg-white text-black font-semibold"
                          : "bg-gray-100"
                      }`}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <Check
                        color={`${
                          selectedIdCateg?.includes(index) ? "red" : "gray"
                        }`}
                      />
                      {item}
                    </div>
